...
export function checkAndFixFilePath(p, name, continueOnError): string {
    if (p) {
...
function arePathEqual(p1, p2): boolean {
    if (!p1 && !p2) return true;
    else if (!p1 || !p2) return false;
    else return path.normalize(p1 || "") === path.normalize(p2 || "");
}

function getAllFiles(rootPath, recursive): string[] {
    let files: string[] = [];
...
export function createZipStream(symbolsPaths: string[], symbolsRoot: string): NodeJS.ReadableStream {
    ...
    symbolsPaths.forEach(rootPath => {
        ...
        for (let i = 0; i < filePaths.length; i++) {
            ...
            tl.debug(`------ Adding zip-entry: ${relativePath} ...`);
...
export function isDsym(s: string): boolean {
...
export function resolveSinglePath(pattern: string, continueOnError: boolean = false, packParentFolder: boolean = false): string {
...
export function resolvePaths(pattern: string, continueOnError: boolean = false, packParentFolder: boolean = false): string[] {
